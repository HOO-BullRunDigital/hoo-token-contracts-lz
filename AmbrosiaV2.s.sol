...
contract DeployAmbrosiaV2 is Script, BaseDeployer {

    address private create2addrAmbrosiaV2;
    ...
    function deployAmbrosiaV2Testnet(uint256 _ambrosiaSalt, uint256 _ambrosiaProxySalt) public setEnvDeploy(Cycle.Test) {
        ambrosiaSalt = bytes32(_ambrosiaSalt);
        ambrosiaProxySalt = bytes32(_ambrosiaProxySalt);

        createDeployMultichainTestnet(true);
    }

    function deployAmbrosiaV2Mainnet(uint256 _ambrosiaSalt, uint256 _ambrosiaProxySalt) public setEnvDeploy(Cycle.Prod) {
        ambrosiaSalt = bytes32(_ambrosiaSalt);
        ambrosiaProxySalt = bytes32(_ambrosiaProxySalt);

        createDeployMultichainMainnet(true);
    }

    function mintAmbrosiaV2TokensTestnet(uint256 amount, address tokenAddress) public setEnvDeploy(Cycle.Test) {
    ...
    function createDeployMultichainTestnet(bool mint) private {
        ...
        for (uint256 i; i < targetChainsTestnet.length;) {
            ...
            deployedContracts[i] = chainDeployAmbrosiaV2(targetChainsTestnet[i].endpoint, true, i);
    ...
    function createDeployMultichainMainnet(bool mint) private {
        ...
        for (uint256 i; i < targetChainsMainnet.length;) {
            ...
            deployedContracts[i] = chainDeployAmbrosiaV2(targetChainsMainnet[i].endpoint, true, i);
    ...
    returns (address deployedContract)
    {
        ...
        require(create2addrAmbrosiaV2 == address(ambrosia), "Implementation address mismatch");

        console2.log("AmbrosiaV2 implementation address:", address(ambrosia), "\n");

        proxyAmbrosia = new UUPSProxy{salt: ambrosiaProxySalt}(
            address(ambrosia), abi.encodeWithSelector(AmbrosiaV2.initialize.selector,"Ambrosia", "AMBR", lzEndpoint, ownerAddress)
        );
        ...
        console2.log("AmbrosiaV2 Proxy address:", address(proxyAmbrosia), "\n");
    ...
    modifier computeCreate2(bytes32 ambrosiaSalt, bytes32 ambrosiaProxySalt, address lzEndpoint) {
        create2addrAmbrosiaV2 = vm.computeCreate2Address(ambrosiaSalt, hashInitCode(type(AmbrosiaV2).creationCode));
        ...
        create2addrProxy = vm.computeCreate2Address(
            ...
            hashInitCode(
                ...
                abi.encode(
                    create2addrAmbrosiaV2, abi.encodeWithSelector(AmbrosiaV2.initialize.selector,"Ambrosia", "AMBR", lzEndpoint, ownerAddress)
                )
            )
        );
    ...
    function mintTokens(uint256 amount, address tokenAddress) private {
        console2.log("Minting AMBR tokens to owner address:", ownerAddress, "\n");
